...
impl Worker {
    pub fn new(v: usize) -> Worker {
        Worker {
            track_value: Rc::new(v),
...
impl Logger for Worker {
    fn warning(&self, msg: &str) {
        ...
        let arr: Vec<&str> = msg.split(": ").collect();
        self.mapped_messages.borrow_mut().insert(String::from(arr[0]), String::from(arr[1]));
    }
